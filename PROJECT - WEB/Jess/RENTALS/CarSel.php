...
$sql = "SELECT carID
        FROM Car 
        WHERE DeletedFlag = 0 AND currentStatus = 'Available'
         ";
...
echo "<div class='listboxJ'><br><select name='listbox' id='listbox' onClick='carpop()'><option >Select Car</option>";
...
while ($row = mysqli_fetch_array($result)) {
    ...
    $_SESSION['RAC']=$row['carID'];

    // Output an option element showing name to the user
    echo "<option value='$id' id='selected' >$id</option>";
}
